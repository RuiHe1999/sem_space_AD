...
# 3.2.2 visualization 
fig, axes = plt.subplots(3, 6, figsize=(48, 24))
axes = axes.flatten()
...
for i, feat in enumerate(variables):
    ...
    axes[i*2].set_ylabel(feat, fontsize=12)
    axes[i*2].legend(loc=1, fontsize=12)
    axes[i*2].set_xticklabels(['English', 'Greek'], fontsize=12)
    axes[i*2].set_yticklabels(axes[i*2].get_yticklabels(), fontsize=12)
    axes[i*2].set_title(f'({chr(65+i*2)}) Group difference in {feat}', fontsize=12)
    ...
    axes[i*2+1].set_title(f'({chr(65+i*2+1)}) Error bar for coefficients for {feat}', fontsize=12)
...
for i, feat in enumerate(sig_feats):
    ...
    axes[i*2].set_xlabel("MMSE", fontsize=12)
    axes[i*2].set_ylabel(feat, fontsize=12)
    axes[i*2].legend(loc=0, fontsize=12)
    axes[i*2].set_yticklabels(axes[0].get_yticklabels(), fontsize=12)
    axes[i*2].set_title(f'({chr(65+i*2)}) Relation between {feat} and MMSE score', fontsize=12)
